...
export default function HomePage() {
  ...
  const [checkDoses, setCheckDoses] = useState([])
  ...
  return (
    <div className="">
      ...
      <div className="gap-5 flex flex-wrap justify-start ml-[4%]">
        {
          loading === false && medicineData.length === 0 ? <h1 className='grid place-items-center h-[72vh] mx-auto  font-mono text-5xl'>No medicines found</h1> : (medicineData.map((item, id) => {
        ...
        w-full sm:w-[48%] md:w-[30%] lg:w-[22%] rounded-2xl shadow-2xl p-4">
                <div className="flex items-center gap-2">
                  <h1 className="text-wrap font-semibold">Today you have to give {item.medicine_name} at {item.schedule[0]?.doses[0]?.time} of {item.schedule[0]?.doses[0]?.dosage} at Day {item.schedule[0]?.day} of Date {item.schedule[0]?.date} <input className="w-4 ml-2 accent-[#03e9f4] h-4" onChange={() => { handleCheckbox(dose._id) }} checked={dose?._id ? checkDoses.includes(dose._id) : false} type="checkbox" /></h1>
                </div>
                <button onClick={() => { handleDeleteDose(dose._id, item._id) }} className="flex gap-2 bg-[#03e9f4] mx-auto  text-black font-semibold w-[44%] px-4  my-2 py-2 rounded transition duration-150 ease-in-out transform active:scale-75 active:shadow-inner shadow-lg">
                  {buttonLoading && <div
